...
// Hoist static Map outside component for O(1) lookups
const createCategoryMatchMap = (products: StorefrontProduct[], categories: CategoryOption[]) => {
  ...
  products.forEach((product) => {
    ...
    keys.forEach((key) => {
      ...
      matchMap.get(key)!.add(product.id)
    })
  })
...
const HeroSection = memo(function HeroSection({ t }: { t: any }) {
  return (
    <motion.div
...
export default function HomePageClient({ products: initialProducts, categories, locale, emptyMessage }: HomePageClientProps) {
  ...
  // Memoize normalized categories once
  ...
  // Optimize category counts using Map for O(1) lookups
  ...
  // Optimize filtering using early exit
  ...
  const handleLayoutClick = useCallback(() => {
    triggerHaptic('light')
    setViewMode((prev) => prev === 'tabs' ? 'bento' : 'tabs')
  }, [])
  ...
  }, [categoriesWithCounts, activeCategory, present])
