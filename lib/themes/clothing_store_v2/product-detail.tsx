...
import { getLocalizedProductText, inferSizesFromTags, mapStorefrontProductToCartProduct } from './utils'
import styles from './styles/theme.module.css'

export function ClothingStoreV2ProductDetail({ product, locale }: ProductDetailProps) {
  ...
  const { name, description, category } = getLocalizedProductText(product, locale)
  ...
  const availableSizes = useMemo(() => inferSizesFromTags(product.tags, locale), [product.tags, locale])
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      title: name,
      text: isPurchasable
        ? t('shareTextWithPrice', { name, price: formatCurrency(product.price) })
        : t('shareText', { name }),
  ...
  return (
    <LayoutGroup>
      <motion.div
        className={cn(
          styles.themeRoot,
          'min-h-screen bg-gradient-to-br from-background via-background to-muted/20 pt-[72px] pb-bottom-nav relative overflow-hidden'
        )}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      >
        ...
        <div className="max-w-md mx-auto lg:max-w-7xl lg:grid lg:grid-cols-2 lg:gap-16 lg:px-8 relative z-10">
          <div className="relative">
            ...
            >
              <ProductImageSlider images={productImages} productName={name} onTryOn={handleTryOn} />
            </motion.div>
          </div>
            ...
            {availableSizes.length > 0 && (
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  <p className="text-sm font-semibold tracking-wide">{t('selectSize')}</p>
                  <AnimatePresence>
                    {selectedSize && (
                      <motion.span
              ...
              <div className="p-6 rounded-3xl bg-gradient-to-br from-muted/50 to-muted/30 backdrop-blur-xl border border-border/50">
                {isPurchasable ? (
                  <div className="space-y-2">
                    <p className="text-sm text-muted-foreground font-medium">{t('totalPrice')}</p>
                  ...
                  <p className="text-xl md:text-2xl font-semibold text-muted-foreground">{t('pricingSoon')}</p>
                )}
              </div>
