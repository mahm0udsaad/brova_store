...
interface CameraCaptureFrontProps {
  onCapture: (file: File) => void;
  onBack?: () => void;
}

function getCameraErrorMessage(err: any): string {
  // iOS/Safari and in-app browsers often fail without prompting.
  ...
  if (name === "NotAllowedError" || name === "PermissionDeniedError") {
    return (
      "Camera permission is blocked for this website. On iPhone: Settings → Safari → Camera → Allow, then in Safari open the site and tap aA → Website Settings → Camera → Allow."
    );
  }
  ...
  // Common iOS message when blocked by browser context / policy.
...
export function CameraCaptureFront({
  onCapture,
  onBack,
}: CameraCaptureFrontProps) {
      ...
      <div className="absolute top-0 left-0 right-0 z-20 px-4 py-6 bg-gradient-to-b from-black/60 to-transparent">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <div className="flex-1 text-center">
            <h2 className="text-white font-semibold">Front View</h2>
            <p className="text-white/70 text-sm">Stand facing the camera</p>
          </div>
          <div className="w-10" />
        </div>
      </div>
      ...
      <div className="flex-1 relative flex items-center justify-center">
        {!capturedImage ? (
          <>
            ...
            {/* Overlay Guide - Larger frame for full body */}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none px-4">
              <div className="relative w-full max-w-sm aspect-[3/4] border-2 border-white/40 rounded-2xl">
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full text-center px-4">
                  <div className="text-white/80 text-sm font-medium bg-black/40 backdrop-blur-sm px-3 py-2 rounded-lg">
